...
from models.vagr import VAGRClassifier
...
class Instructor:
    ...
    def __init__(self, opt, config):
        ...
        if 'bert' in opt.model_name:
            ...
            opt.tok_size = len(embedding_matrix)
            opt.post_size = len(post_vocab)
            opt.pos_size = len(pos_vocab)

            vocab_help = (post_vocab, pos_vocab, dep_vocab, pol_vocab)
            self.model = opt.model_class(embedding_matrix, self.opt, config).to(opt.device)
            trainset = SentenceDataset(opt.dataset_file['train'], tokenizer, opt=opt, vocab_help=vocab_help, flag='train')
            testset = SentenceDataset(opt.dataset_file['test'], tokenizer, opt=opt, vocab_help=vocab_help, flag='test')
    ...
    def _train(self, criterion1, criterion2, optimizer, max_test_acc_overall=0):
        global vaeout, mean, std, loss_cevae, loss_kl, penalbi
        kldiv = GaussianKLDiv()
        max_test_acc = 0
        max_f1 = 0
        global_step = 0
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                outputs, penal, vaeout, mean, std = self.model(inputs)
                # outputs, penal = self.model(inputs)
                targets = sample_batched['polarity'].to(self.opt.device)
                loss_ce = criterion1(outputs, targets) + penal
                loss = 0
                loss += loss_ce

                # trg_output = inputs[1].view(-1).clone()
                trg_output = torch.zeros(inputs[1].size(0), inputs[1].size(1)).cuda()
                for i in range(inputs[1].size(0)):
                    position = inputs[6][i].nonzero()
                    for j in range(position.size(0)):
                        trg_output[i][position[j].item()] = inputs[1][i][j]
                trg_output = trg_output.view(-1).long()
                # vaeout = torch.mul(vaeout, inputs[6].unsqueeze(dim=2))
                # trg_output[trg_output >= 1] = 1

                # loss += penal
                output_size = vaeout.size(-1)
                vaeout = vaeout.view(-1, output_size)
                loss_cevae = criterion2(vaeout, trg_output)
                loss_kl = kldiv(mean, std)
                loss += loss_cevae * self.opt.vae_lambda1 + loss_kl * self.opt.vae_lambda2
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    if test_acc > max_test_acc:
                        ...
                        if test_acc > max_test_acc_overall:
                            if not os.path.exists('./VAGR/state_dict'):
                                os.mkdir('./VAGR/state_dict')
                            model_path = './VAGR/state_dict/{}_{}_acc_{:.4f}_f1_{:.4f}'.format(self.opt.model_name,
                                                                                                  self.opt.dataset,
                                                                                                  test_acc, f1)
                    ...
                    logger.info('loss: {:.4f}, CEloss: {:.4f}, CEloss(aspect): {:.4f}, KLloss: {:.4f}, acc: {:.4f}, '
                                  'test_acc: {:.4f}, f1: {:.4f}'.format(loss.item(), loss_ce.item(), loss_cevae.item(), loss_kl.item(),
                                         train_acc, test_acc, f1))
                    # logger.info('loss: {:.4f}, CEloss: {:.4f}, acc: {:.4f}, '
                    #             'test_acc: {:.4f}, f1: {:.4f}'.format(loss.item(), loss_ce.item(),
                    #                                                   train_acc, test_acc, f1))
    ...
    def _evaluate(self, show_results=False):
        ...
        with torch.no_grad():
            for batch, sample_batched in enumerate(self.test_dataloader):
                ...
                outputs, penal, _, _, _ = self.model(inputs)
                # outputs, penal = self.model(inputs)
    ...
    def run(self):
        ...
        if 'bert' not in self.opt.model_name:
            self._reset_params()
        max_test_acc, max_f1, model_path = self._train(criterion1, criterion2, optimizer, max_test_acc_overall)
...
def main():
    ...
    dataset_files = {
        'restaurant': {
            'train': '../VAGR/dataset/Restaurants_corenlp/train.json',
            'test': '../VAGR/dataset/Restaurants_corenlp/test.json',
        },
        'laptop': {
            'train': '../VAGR/dataset/Laptops_corenlp/train.json',
            'test': '../VAGR/dataset/Laptops_corenlp/test.json'
        },
        'twitter': {
            'train': '../VAGR/dataset/Tweets_corenlp/train.json',
            'test': '../VAGR/dataset/Tweets_corenlp/test.json',
        },
        'res15': {
            'train': '../VAGR/dataset/Res15/train.json',
            'test': '../VAGR/dataset/Res15/test.json',
        },
        'res16': {
            'train': '../VAGR/dataset/Res16/train.json',
            'test': '../VAGR/dataset/Res16/test.json',
        }
    }
    ...
    input_colses = {
        ...
        'vagr': ['text', 'aspect', 'pos', 'head', 'deprel', 'post', 'mask', 'length', 'adj'],
    ...
    vocab_dir = {
        'laptop': '../VAGR/dataset/Laptops_corenlp',
        'restaurant': '../VAGR/dataset/Restaurants_corenlp',
        'twitter': '../VAGR/dataset/Twitters_corenlp',
        'res15': '../VAGR/dataset/Res15',
        'res16': '../VAGR/dataset/Res16',
    }

    # Hyperparameters
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_name', default='vagr', type=str, help=', '.join(model_classes.keys()))
    parser.add_argument('--dataset', default='res16', type=str, help=', '.join(dataset_files.keys()))
    ...
    parser.add_argument("--weight_decay", default=0.000001, type=float, help="Weight decay if we apply some.")
    parser.add_argument('--vocab_dir', type=str, default='../VAGR/dataset/Laptops_corenlp')
    ...
    # ensemble parameters
    parser.add_argument('--ensemble_alpha', default=0.5, type=float)
    parser.add_argument('--prune_rate', default=0.1, type=float)
    ...
    parser.add_argument('--VAE', default=True)
    ...
    if not os.path.exists('./VAGR/log'):
        os.makedirs('./VAGR/log', mode=0o777)
    log_file = '{}-{}-{}.log'.format(opt.model_name, opt.dataset, strftime("%Y-%m-%d_%H:%M:%S", localtime()))
    logger.addHandler(logging.FileHandler("%s/%s" % ('./VAGR/log', log_file)))

    config = yaml.safe_load(open(opt.config, 'r', encoding='utf-8'))
    # config = ""
    ins = Instructor(opt, config)
    ins.run()
