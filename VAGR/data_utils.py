...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help, flag):
        if opt.dataset == 'laptop':
            road = './dataset/Laptops_corenlp/Laptop_{}.json'
        elif opt.dataset == 'restaurant':
            road = './dataset/Restaurants_corenlp/Restaurant_{}.json'
        elif opt.dataset == 'twitter':
            road = './dataset/Tweets_corenlp/Twitter_{}.json'
        elif opt.dataset == 'res15':
            road = './dataset/Res15/Res15_{}.json'
        elif opt.dataset == 'res16':
            road = './dataset/Res16/Res16_{}.json'
        if os.path.exists(road.format(flag)) and opt.parseadj:
            with open(road.format(flag)) as jsonfile:
                self.data = json.load(jsonfile)

            for d in self.data:
                d["text"] = np.asarray(d["text"], dtype='int64')
                d["aspect"] = np.asarray(d["aspect"], dtype='int64')
                d["post"] = np.asarray(d["post"], dtype='int64')
                d["pos"] = np.asarray(d["pos"], dtype='int64')
                d["deprel"] = np.asarray(d["deprel"], dtype='int64')
                d["head"] = np.asarray(d["head"], dtype='int64')
                d["adj"] = np.asarray(d["adj"], dtype='int64')
                d["mask"] = np.asarray(d["mask"], dtype='int64')
                d["length"] = np.asarray(d["length"], dtype='int64')
                d["polarity"] = np.asarray(d["polarity"], dtype='int64')
        else:
            parse = ParseData
            ...
            for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
                ...
                post = tokenizer.pad_sequence(post, pad_id=opt.pad_id, maxlen=opt.max_length, dtype='int64',
                                              padding='post',
                                              truncating='post')
                pos = [pos_vocab.stoi.get(t, pos_vocab.unk_index) for t in obj['pos']]
                pos = tokenizer.pad_sequence(pos, pad_id=opt.pad_id, maxlen=opt.max_length, dtype='int64',
                                             padding='post',
                                             truncating='post')
                ...
                if opt.parsehead:
                    ...
                    head = tokenizer.pad_sequence(syntree2head[0], pad_id=opt.pad_id, maxlen=opt.max_length,
                                                  dtype='int64',
                                                  padding='post', truncating='post')
                else:
            ...
            if opt.parseadj:
                with open(road.format(flag), "w") as f:
                    json.dump(self.data, fp=f, indent=6)
                f.close()
...
def build_embedding_matrix(vocab, embed_dim, data_file):
    if os.path.exists(data_file):
        ...
        fname = '../VAGR/glove/glove.840B.300d.txt'
